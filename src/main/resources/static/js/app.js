...
/* ---------- 可搜索下拉组件（无第三方依赖） ----------
 * 将原生 <select> 转成"输入框 + 下拉列表"的可搜索选择器：
 * - 原生 select 保留并隐藏：作为选项数据源（仍由 fillTenantSelects 等填充）和提交值载体，
 *   因此读取 .value 的现有逻辑无需改动；
 * - 输入关键字按显示文本模糊过滤（不区分大小写），点击选项或回车选中首项；
 * - 对外暴露 setValue(code)（如默认跟随全局）、getValue()。
 * @param selectId   原生 select 的 id
 * @param widthClass 外层容器宽度样式，如 'w-40'、'w-full'（默认 w-40）
 * @param placeholder 输入框占位文案（默认"搜索租户..."）
 */
function initSearchableSelect(selectId, widthClass, placeholder) {
    ...
    wrap.className = 'relative ' + (widthClass || 'w-40');
    const input = document.createElement('input');
    input.type = 'text';
    input.id = selectId + 'Search';
    input.placeholder = placeholder || '搜索租户...';
    input.autocomplete = 'off';
    input.className = 'w-full px-3 py-2 text-sm bg-dark-850 border border-white/10 rounded-lg focus:border-primary-500 focus:outline-none transition placeholder-gray-500';
    const list = document.createElement('div');
    list.className = 'hidden absolute left-0 right-0 z-30 mt-1 max-h-56 overflow-y-auto bg-dark-900 border border-white/10 rounded-lg shadow-xl';
    ...
    const render = (keyword) => {
        ...
        Array.from(sel.options).forEach(opt => {
            if (opt.value === '') return; // "请选择租户"等空值占位项不可点击
...
async function initTenantSystemSelects() {
    ...
    fillSystemSelects(['globalSystem', 'uploadSystem', 'urlSystem', 'apiKeySystem'], false);
    ...
    initTenantSearchable('urlTenant', 'w-full');
    initTenantSearchable('docFilterTenant', 'w-40');
    initTenantSearchable('apiKeyTenant', 'w-40');
    // 恢复上次选择的租户/系统（仅全局）
    try {
        const c = JSON.parse(localStorage.getItem(CFG_KEY) || '{}');
        if (c.tenant) setTenantValue('globalTenant', c.tenant);
