...
- **多轮重答支持**：用户要求"重新回答上一个问题 / 换个说法"等指令时，即使本轮知识库检索无结果，也会基于历史对话中的原问答重新组织作答（历史始终随对话传入，检索为空时自动兜底）
...
- **文档管理筛选**：文档列表页支持按租户 + 系统类型独立筛选（独立于顶部全局选择，变更即刷新列表）
...
- **API Key 生成界面**：左侧导航「API Key 管理」→ 备注为文本框（必填）；租户为**可搜索下拉**（选项来自租户配置表，输入关键字模糊过滤，不含租户 `0`）；系统类型为**下拉选择**（选项来自 `js/config.js`）；两下拉默认跟随顶部全局选择（全局租户为 `0` 时需手动选择具体租户）
...
- **租户/系统必填**：生成时必须绑定 `tenantCode`（具体租户编码）与 `systemType`（系统类型），对话/上传时以此归属； **不允许绑定租户
  `0`**（超管全局租户），传 `0` 返回 `400`；管理界面已提供下拉选择（租户为可搜索下拉）
...
│       ├── js/app.js                       # 前端交互逻辑（401 拦截/登录/CSRF/API Key 管理/可搜索下拉）
│       ├── js/config.js                    # 前端配置（系统类型下拉选项 window.MOTCS_CONFIG.systems，部署可改）
│       ├── js/marked.min.js                # Markdown 渲染
│       ├── js/tailwindcss.js               # Tailwind CSS（Play CDN，动态编译 class）
