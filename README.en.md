...
- **Selectable models**: dropdown below the input toggles `deepseek-v4-flash-0731` / `glm-5.3-flash` /
  `ernie-4.5-turbo-128k` (options are served by `GET /ai/v1/provider` based on the active AI platform, preventing
  invalid selections)
...
- **docCode auto-generated by the backend**: format `yyyyMMdd` + capitalized first letter of file type (docx->D, xlsx->
  X, ppt->P, txt->T, no extension->F) + 3-digit sequence (e.g. `20260920D001`); a process lock serializes "query today's
  max code +1 and persist" to avoid concurrent duplicates
- **On-disk rename with original-name mapping**: files are stored as `docCode.ext` on disk (the UI always shows the
  original file name); `document_info.stored_file_name` records the disk name, and chunking / graph / vectors / deletion
  all use the stored name; legacy rows are backfilled from `file_path` at startup
- **Stuck auto-retry**: a doc stuck in `PROCESSING` for over 2 hours and never retried has its vectors and graph
  cleared, then is reprocessed once from the on-disk file (tracked by `retry_count`, only one auto retry); if the source
  file is missing it is marked Failed with a reason; docs in `PROCESSING` can also be deleted
...
| `AI_CHAT_MODEL`          | `glm-5.3-flash`                                             | Chat model (see [Model Selection](#model-selection)) |
...
| `AI_CHAT_MODELS`         | `deepseek-v4-flash-0731,glm-5.3-flash,ernie-4.5-turbo-128k` | Comma-separated selectable models                    |
...
| Variable            | Default                                     | Description                                                                                            |
|---------------------|---------------------------------------------|--------------------------------------------------------------------------------------------------------|
| `FILE_UPLOAD_DIR`   | `./uploads`                                 | File upload dir (for containers, bind-mount to a host path such as `/mnt/graphrag/uploads` to persist) |
...
The `document_info` metadata table is auto-created from the entity. Key columns:

- `file_name`: original file name (shown in UI); `stored_file_name`: on-disk name (`docCode.ext`, used by
  chunking/graph/vectors/deletion), backfilled from `file_path` for legacy rows at startup
- `retry_count`: stuck-auto-retry flag (0 = not retried, 1 = retried once), used by the startup task to avoid docs
...
| Method   | Path                              | Access | Description                                                                                                                   |
|----------|-----------------------------------|--------|-------------------------------------------------------------------------------------------------------------------------------|
| `POST`   | `/documents/v1/upload`            | Admin  | Upload (multipart/form-data: file/title/description/tenantCode/systemType/userId; `docCode` is auto-generated by the backend) |
...
`model` values: `deepseek-v4-flash-0731` / `glm-5.3-flash` / `ernie-4.5-turbo-128k` (defaults to the configured model).
...
The chat endpoints support switching between `deepseek-v4-flash-0731`, `glm-5.3-flash` and `ernie-4.5-turbo-128k`:

| Model                    | Characteristics                       | Default                          |
|--------------------------|---------------------------------------|----------------------------------|
| `deepseek-v4-flash-0731` | General chat, fast, cost-effective    | Frontend dropdown default        |
| `glm-5.3-flash`          | Deep reasoning, for complex questions | Config default (`AI_CHAT_MODEL`) |
| `ernie-4.5-turbo-128k`   | Lightweight fast variant              | -                                |
...
A: The batch timeout budget now matches `AI_TIMEOUT` (300s per batch). Thinking models (`glm-5.3-flash`) respond
slowly by design; if a single call exceeds ~5 min, check Qianfan load or switch to `deepseek-v4-flash-0731`.
...
| Project                                        | Purpose                                                                          |
|------------------------------------------------|----------------------------------------------------------------------------------|
| [Baidu Qianfan](https://qianfan.baidubce.com/) | deepseek-v4-flash-0731 chat models + bge-large-zh embeddings (OpenAI-compatible) |
