...
- **docCode auto-generated by the backend**: format `yyyyMMdd` + capitalized first letter of file type (docx->D, xlsx->X, ppt->P, txt->T, no extension->F) + 3-digit sequence (e.g. `20260920D001`); a process lock serializes "query today's max code +1 and persist" to avoid concurrent duplicates
- **On-disk rename with original-name mapping**: files are stored as `docCode.ext` on disk (the UI always shows the original file name); `document_info.stored_file_name` records the disk name, and chunking / graph / vectors / deletion all use the stored name; legacy rows are backfilled from `file_path` at startup
- **Stuck auto-retry**: a doc stuck in `PROCESSING` for over 2 hours and never retried has its vectors and graph cleared, then is reprocessed once from the on-disk file (tracked by `retry_count`, only one auto retry); if the source file is missing it is marked Failed with a reason; docs in `PROCESSING` can also be deleted
...
- `file_name`: original file name (shown in UI); `stored_file_name`: on-disk name (`docCode.ext`, used by chunking/graph/vectors/deletion), backfilled from `file_path` for legacy rows at startup
- `retry_count`: stuck-auto-retry flag (0 = not retried, 1 = retried once), used by the startup task to avoid docs staying in Processing forever
