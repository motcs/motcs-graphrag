...
登录管理界面 → 左侧导航「API Key 管理」→ 填写 **备注（必填）**，选择 **租户（必填，可搜索下拉）** 与 **系统类型（必填，下拉）** → 点击「生成新 Key」。

界面操作说明：

| 输入项 | 方式 | 说明 |
|--------|------|------|
| 备注 | 文本框 | 填写用途（如对接方名称/系统名），必填 |
| 租户 | **可搜索下拉** | 点击展开、输入关键字（如"长安"）即时模糊过滤租户名称后选择；选项来自后端租户配置表（`GET /documents/v1/tenants`），**不含租户 `0`**（不允许绑定超管全局租户） |
| 系统类型 | 下拉选择 | 选项来自前端配置 `js/config.js`（人大/政协/党建/社会工作部/数智统战/综合平台等），与租户共同决定检索的知识库范围 |

> 两个下拉默认跟随顶部全局选择；全局租户为 `0`（全部租户）时，需手动选择具体租户后才能生成。
...
>
> 字段取值与界面下拉一致：`tenantCode` 取租户配置表中的具体租户编码（不可为 `0`）；`systemType` 取值见
> `js/config.js` 的 `window.MOTCS_CONFIG.systems`（`congress` / `cppcc` / `party` / `msw` / `digital` / `other`）。
